...
public interface PatientRepository extends JpaRepository<Patient,Long> {
   ...
   @Query("select new com.Spring0To1LearningProjectionAndMoreModule3.HospitalManagementSystem.dto.CPatientInfo(p.id, p.name) " +
            "from Patient p")
    List<CPatientInfo> getAllPatientInfoC();

    @Query("select new com.Spring0To1LearningProjectionAndMoreModule3.HospitalManagementSystem.dto.BloodGroupStats(p.bloodGroup, COUNT(p)) " +
            "from Patient p " +
